...
router.post('/login', async (req, res) => {
    try {
        ...
        const nextFromSession = req.session.afterLoginRedirect;
        const nextUrl = nextFromSession && String(nextFromSession).startsWith('/') ? nextFromSession : null;

        if (user.user_type === 'professional') {
            const { data: prof } = await supabase.from('professionals').select('*').eq('user_id', user.id).maybeSingle();
            if (!basicProfessionalProfileComplete(prof)) { req.session.afterLoginRedirect = null; return res.redirect('/auth/completar-perfil'); }
            if (!prof?.profile_completed) { req.session.afterLoginRedirect = null; return res.redirect('/profissional/onboarding?step=1'); }
            req.session.afterLoginRedirect = null;
...
router.get('/google/callback', passport.authenticate('google', {
    ...
    try {
        ...
        if (user.user_type === 'professional') {
            ...
            if (user._wasNew || !basicProfessionalProfileComplete(prof)) { req.session.afterLoginRedirect = null; return res.redirect('/auth/completar-perfil'); }
            if (!prof?.profile_completed) { req.session.afterLoginRedirect = null; return res.redirect('/profissional/onboarding?step=1'); }
            req.session.afterLoginRedirect = null;
            return res.redirect(nextUrl && nextUrl.startsWith('/profissional') ? nextUrl : '/profissional/dashboard');
        }
