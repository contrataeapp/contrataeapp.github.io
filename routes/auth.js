...
router.post('/cadastro', async (req, res) => {
    try {
        const { full_name, email, password, password_confirm, user_type, next } = req.body;
        ...
        if (cleanName.length < 5 || cleanName.split(' ').length < 2 || cleanName.split(' ').some(part => part.length < 2)) {
            return res.render('auth/cadastro', { erro: 'Informe nome e sobrenome válidos para criar a conta', userType: user_type || 'client', next: nextUrl || '' });
        }

        if (password !== password_confirm) {
            return res.render('auth/cadastro', { erro: 'As senhas não coincidem', userType: user_type || 'client', next: nextUrl || '' });
        }
        ...
        if (userError) {
            ...
            if (userError.code === '23505') {
                return res.render('auth/cadastro', { erro: 'E-mail já cadastrado', userType: user_type || 'client', next: nextUrl || '' });
            }
            return res.render('auth/cadastro', { erro: 'Erro ao criar conta: ' + userError.message, userType: user_type || 'client', next: nextUrl || '' });
        }
        ...
        req.session.regenerate((sessionErr) => {
            if (sessionErr) {
                ...
                return res.render('auth/cadastro', { erro: 'Erro ao iniciar sessão', userType: req.body.user_type || 'client', next: nextUrl || '' });
            }
            ...
            req.session.save(() => {
                ...
                return res.redirect(nextUrl && (nextUrl.startsWith('/cliente') || nextUrl.startsWith('/perfil') || nextUrl.startsWith('/categoria')) ? nextUrl : '/cliente/dashboard');
            });
        });
        return;
    } catch (err) {
        console.error(err);
        res.render('auth/cadastro', { erro: 'Erro ao criar conta', userType: req.body.user_type || 'client', next: (req.body.next || '') });
    }
});
...
router.post('/login', async (req, res) => {
    try {
        ...
        req.session.regenerate(async (sessionErr) => {
            ...
            return finishRedirect(nextUrl && (nextUrl.startsWith('/cliente') || nextUrl.startsWith('/perfil') || nextUrl.startsWith('/categoria')) ? nextUrl : '/cliente/dashboard');
        });
    } catch (err) {
        console.error(err);
