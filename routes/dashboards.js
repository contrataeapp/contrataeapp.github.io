...
async function handleProfessionalOnboardingSave(req, res) {
    ...
    if (Object.keys(userUpdates).length) {
        ...
        if (Object.prototype.hasOwnProperty.call(userUpdates, 'avatar_url')) req.session.avatarUrl = userUpdates.avatar_url || null;
    }
...
router.post('/profissional/profissoes/portfolio/substituir', requireProfessional, upload.single('profession_portfolio_image'), catchAsync(async (req, res) => {
    const id = compactText(req.body?.id);
    const slotNumber = Number(req.body?.slot || 0);
    if (!id || !req.file) return res.redirect('/profissional/dashboard?tab=portfolio&slot=' + (slotNumber || 1) + '&error=' + encodeURIComponent('Escolha uma imagem para substituir a foto atual.'));
    const ext = (req.file.originalname.split('.').pop() || 'jpg').toLowerCase();
    if (!['jpg','jpeg','png','webp'].includes(ext)) return res.redirect('/profissional/dashboard?tab=portfolio&slot=' + (slotNumber || 1) + '&error=' + encodeURIComponent('Use uma imagem JPG, PNG ou WEBP.'));
    const { data: item, error: loadError } = await supabaseAdmin.from('professional_profession_portfolio').select('id, profession_profile_id, image_url, position, draft_caption, caption, is_published, pending_delete').eq('id', id).eq('professional_id', req.session.userId).maybeSingle();
    if (loadError) throw loadError;
    if (!item || item.pending_delete) return res.redirect('/profissional/dashboard?tab=portfolio&slot=' + (slotNumber || 1) + '&error=' + encodeURIComponent('Imagem não encontrada ou já marcada para remoção.'));
    const path = `portfolio/profession_${req.session.userId}_${slotNumber}_replace_${Date.now()}_${Math.random().toString(36).slice(2, 7)}.${ext}`;
    const imageUrl = await uploadToBucket('avatars', path, req.file);
    const preservedCaption = compactText(item.draft_caption ?? item.caption ?? '').slice(0, 180) || null;
    if (item.is_published === false) {
        const { error: updateError } = await supabaseAdmin.from('professional_profession_portfolio').update({ image_url: imageUrl, draft_caption: preservedCaption, pending_delete: false, updated_at: new Date().toISOString() }).eq('id', id).eq('professional_id', req.session.userId);
        if (updateError) throw updateError;
    } else {
        const { error: insertError } = await supabaseAdmin.from('professional_profession_portfolio').insert({
            professional_id: req.session.userId,
            profession_profile_id: item.profession_profile_id,
            slot: slotNumber,
            image_url: imageUrl,
            position: Number(item.position || 0),
            caption: null,
            draft_caption: preservedCaption,
            is_published: false,
            pending_delete: false
        });
        if (insertError) throw insertError;
        const { error: oldError } = await supabaseAdmin.from('professional_profession_portfolio').update({ pending_delete: true, updated_at: new Date().toISOString() }).eq('id', id).eq('professional_id', req.session.userId);
        if (oldError) throw oldError;
    }
    return res.redirect('/profissional/dashboard?tab=portfolio&slot=' + (slotNumber || 1) + '&success=' + encodeURIComponent('Imagem substituída no rascunho. A foto publicada continua visível até você publicar; posição e legenda foram preservadas.') + '#portfolio-workspace-' + (slotNumber || 1));
}));
...
router.post('/profissional/profissoes/portfolio/desfazer-remocao', requireProfessional, catchAsync(async (req, res) => {
    ...
    // Se a remoção foi criada por "Substituir foto", existe uma foto nova em
    // rascunho ocupando a mesma posição. Ao desfazer, descartamos essa substituta
    // e restauramos a foto publicada antiga, evitando 11 imagens ativas por engano.
    const { data: current, error: loadError } = await supabaseAdmin
        .from('professional_profession_portfolio')
        .select('id, profession_profile_id, position, is_published, pending_delete')
        .eq('id', id)
        .eq('professional_id', req.session.userId)
        .maybeSingle();
    if (loadError) throw loadError;
    if (current?.pending_delete === true && current?.is_published !== false && current?.profession_profile_id) {
        const { data: replacements, error: replacementError } = await supabaseAdmin
            .from('professional_profession_portfolio')
            .select('id')
            .eq('professional_id', req.session.userId)
            .eq('profession_profile_id', current.profession_profile_id)
            .eq('position', Number(current.position || 0))
            .eq('is_published', false)
            .eq('pending_delete', false);
        if (replacementError) throw replacementError;
        const replacementIds = (replacements || []).map(row => row.id).filter(Boolean);
        if (replacementIds.length) {
            const { error: deleteReplacementError } = await supabaseAdmin
                .from('professional_profession_portfolio')
                .delete()
                .in('id', replacementIds)
                .eq('professional_id', req.session.userId);
            if (deleteReplacementError) throw deleteReplacementError;
        }
    }
    ...
    const { error } = await supabaseAdmin
        ...
        .eq('professional_id', req.session.userId);
    if (error) throw error;
    return res.redirect('/profissional/dashboard?tab=portfolio&slot=' + (slotNumber || 1) + '&success=' + encodeURIComponent('Remoção ou substituição cancelada.') + '#portfolio-workspace-' + (slotNumber || 1));
}));
