...
const DEFAULT_PLATFORM_SETTINGS = Object.freeze({
    id: 'main',
    contact_email: 'time.contratae@gmail.com',
    location_text: 'Taquaritinga - SP',
    facebook_url: '',
    ...
    whatsapp_message: 'Olá, encontrei o Contrataê e gostaria de falar com a equipe.'
});
let platformSettingsCache = { value: null, expiresAt: 0 };
...
function normalizePlatformSettings(row) {
    ...
    return {
        ...
        ...value,
        facebook_active: Boolean(value.facebook_active),
        instagram_active: Boolean(value.instagram_active),
        x_active: Boolean(value.x_active),
        whatsapp_active: Boolean(value.whatsapp_active)
    };
}
...
function normalizeInstitutionalPhone(value) {
    let digits = String(value || '').replace(/\D/g, '').slice(0, 15);
    // Facilita o cadastro no Brasil: DDD + número recebe o DDI 55 automaticamente.
    if ((digits.length === 10 || digits.length === 11) && !digits.startsWith('55')) digits = `55${digits}`;
    return digits.slice(0, 15);
}

function buildInstitutionalWhatsAppUrl(settings) {
    const number = normalizeInstitutionalPhone(settings?.whatsapp_number);
    if (!number) return '';
    const message = String(settings?.whatsapp_message || DEFAULT_PLATFORM_SETTINGS.whatsapp_message).trim();
    return `https://wa.me/${number}${message ? `?text=${encodeURIComponent(message)}` : ''}`;
}

async function loadPlatformSettings({ force = false } = {}) {
    const now = Date.now();
    if (!force && platformSettingsCache.value && platformSettingsCache.expiresAt > now) {
        return platformSettingsCache.value;
    }
    let settings = normalizePlatformSettings(null);
    try {
        ...
        settings = normalizePlatformSettings(data);
    } catch (err) {
        const msg = String(err?.message || err || '');
        if (!/platform_settings|relation .* does not exist|schema cache/i.test(msg)) {
            console.warn('Aviso: não foi possível carregar configurações institucionais:', msg);
        }
    }
    settings.facebook_url = normalizeExternalUrl(settings.facebook_url);
    settings.instagram_url = normalizeExternalUrl(settings.instagram_url);
    settings.x_url = normalizeExternalUrl(settings.x_url);
    settings.whatsapp_url = buildInstitutionalWhatsAppUrl(settings);
    platformSettingsCache = { value: settings, expiresAt: now + 60_000 };
    return settings;
}

function invalidatePlatformSettingsCache() {
    platformSettingsCache = { value: null, expiresAt: 0 };
}
...
async function trackProfessionalProfileEvent({ professionalId, professionSlug, professionName, eventType, req }) {
    try {
        ...
        if (String(req?.query?.ownerView || '') === '1') return;
...
app.get('/favicon.ico', (req, res) => res.sendFile(path.join(__dirname, 'public', 'imagens', 'app', 'favicon.png')));
...
// Health check extremamente leve para o Render. Não consulta banco, sessão nem templates.
...
app.use(async (req, res, next) => {
    try {
        // Sessões antigas podem não ter avatarUrl. Busca uma vez e passa a reaproveitar.
        if (req.session?.userId && typeof req.session.avatarUrl === 'undefined') {
            const { data: sessionUser } = await supabase
                .from('users')
                .select('avatar_url')
                .eq('id', req.session.userId)
                .maybeSingle();
            req.session.avatarUrl = sessionUser?.avatar_url || null;
        }
        res.locals.avatarUrl = req.session?.avatarUrl || res.locals.avatarUrl || null;
        res.locals.platformSettings = await loadPlatformSettings();
    } catch (err) {
        res.locals.platformSettings = normalizePlatformSettings(null);
        res.locals.avatarUrl = req.session?.avatarUrl || null;
    }
    next();
});
app.use((req, res, next) => {
...
app.get('/api/profissional/contatos-pendentes', requireProfessional, async (req, res) => {
    try {
        ...
        const requestedStatus = scope === 'all' ? null : 'pending';
        ...
        // A foto do cliente não é duplicada em contact_leads. Buscamos o avatar atual pelo client_id,
        // evitando ocupar o banco com cópias de imagens e mantendo o lead ligado à mesma identidade.
        const clientIds = [...new Set((leads || [])
            .map(lead => String(lead?.client_id || '').trim())
            .filter(id => isUuid(id)))];
        ...
        if (clientIds.length) {
            const { data: clients, error: clientsError } = await supabaseAdmin
                ...
                .in('id', clientIds);
            if (!clientsError) {
                (clients || []).forEach(client => clientById.set(String(client.id), client));
            }
        }

        const hydratedLeads = (leads || []).map(lead => {
            const client = clientById.get(String(lead?.client_id || '')) || null;
            return {
                ...lead,
                client_name: lead.client_name || client?.full_name || 'Cliente Contrataê',
                client_email: lead.client_email || client?.email || '',
                client_avatar_url: client?.avatar_url || null
            };
        });

        return res.json({ ok: true, leads: hydratedLeads });
    } catch (err) {
...
app.get('/api/admin/platform-settings', checkAdminAPI, async (req, res) => {
    try {
        const { data, error } = await supabaseAdmin
            .from('platform_settings')
            .select('*')
            ...
            .maybeSingle();
        if (error) throw error;
        const settings = normalizePlatformSettings(data);
        settings.whatsapp_url = buildInstitutionalWhatsAppUrl(settings);
        platformSettingsCache = { value: settings, expiresAt: Date.now() + 60_000 };
        res.json({ ok: true, settings });
    } catch (err) {
        console.error('Erro ao carregar configurações da plataforma:', err);
        const msg = String(err?.message || '');
        const setupRequired = /platform_settings|relation .* does not exist|schema cache/i.test(msg);
        res.status(setupRequired ? 503 : 500).json({
            ok: false,
            setup_required: setupRequired,
            erro: setupRequired
                ? 'A tabela de configurações ainda não existe. Execute o SQL_V11_3_43_CONFIGURACOES_PLATAFORMA.sql no Supabase.'
                : 'Não foi possível carregar as configurações.'
        });
    }
});

app.put('/api/admin/platform-settings', checkAdminAPI, async (req, res) => {
    try {
        const body = req.body || {};
        const payload = {
            id: 'main',
            contact_email: String(body.contact_email || '').trim().slice(0, 240),
            location_text: String(body.location_text || '').trim().slice(0, 300),
            facebook_url: normalizeExternalUrl(body.facebook_url),
            facebook_active: Boolean(body.facebook_active),
            instagram_url: normalizeExternalUrl(body.instagram_url),
            instagram_active: Boolean(body.instagram_active),
            x_url: normalizeExternalUrl(body.x_url),
            x_active: Boolean(body.x_active),
            whatsapp_number: normalizeInstitutionalPhone(body.whatsapp_number),
            whatsapp_active: Boolean(body.whatsapp_active),
            whatsapp_message: String(body.whatsapp_message || DEFAULT_PLATFORM_SETTINGS.whatsapp_message).trim().slice(0, 500),
            updated_at: new Date().toISOString()
        };

        if (payload.contact_email && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(payload.contact_email)) {
            return res.status(400).json({ ok: false, erro: 'Informe um e-mail institucional válido.' });
        }
        const { data, error } = await supabaseAdmin
            .from('platform_settings')
...
app.get('/outros', catchAsync(async (req, res) => {
    ...
    res.render('outros', { banners, currentPage: 'outros', extraCategories, searchQuery: String(req.query.busca || req.query.q || '').trim() });
}));
...
// Busca pública incremental usada pelo cabeçalho, home e navegação mobile.
app.get('/api/public/search-suggestions', async (req, res) => {
    try {
        ...
        const aliases = [
            { name: 'Pintores', slug: 'pintores', terms: ['pintor','pintores','pintura'] },
            { name: 'Pedreiros', slug: 'pedreiros', terms: ['pedreiro','pedreiros','obra','alvenaria'] },
            { name: 'Eletricistas', slug: 'eletricistas', terms: ['eletricista','eletricistas','eletrica','elétrica'] },
            { name: 'Encanadores', slug: 'encanadores', terms: ['encanador','encanadores','hidraulica','hidráulica'] }
        ];

        const [{ data: categories }, { data: professionals }] = await Promise.all([
            supabase.from('categories').select('id,name,slug,icon_url').order('name').limit(80),
            supabase.from('professionals')
                .select('user_id,city,state,status,users(full_name,avatar_url),categories(name,slug)')
                .eq('status', 'active')
                .limit(80)
        ]);
        ...
        const score = value => {
            ...
            if (query.includes(text.replace(/s$/, ''))) return 3;
            return 99;
        };

        const categoryMap = new Map();
        aliases.forEach(item => {
            const s = Math.min(score(item.name), ...item.terms.map(score));
            if (s < 99) categoryMap.set(item.slug, {
                type: 'category',
                label: item.name,
                subtitle: 'Ver profissionais desta categoria',
                url: `/categoria/${item.slug}`,
                icon: item.slug === 'pintores' ? 'fa-paint-roller' : item.slug === 'pedreiros' ? 'fa-trowel-bricks' : item.slug === 'eletricistas' ? 'fa-bolt' : 'fa-wrench',
                score: s
            });
        });
        (categories || []).forEach(cat => {
            const s = Math.min(score(cat.name), score(cat.slug));
            if (s < 99 && !categoryMap.has(cat.slug)) categoryMap.set(cat.slug, {
                type: 'category',
                label: cat.name,
                subtitle: 'Ver profissionais desta categoria',
                url: `/categoria/${cat.slug}`,
                icon: String(cat.icon_url || '').includes('fa-') ? String(cat.icon_url).split(/\s+/).find(x => x.startsWith('fa-')) || 'fa-briefcase' : 'fa-briefcase',
                score: s
            });
        });

        const professionalResults = (professionals || []).map(row => {
            const name = row.users?.full_name || '';
            const profession = row.categories?.name || '';
            const s = Math.min(score(name), score(profession), score(`${name} ${profession}`));
            if (s >= 99) return null;
            const place = [row.city, row.state].filter(Boolean).join(' - ');
            return {
                type: 'professional',
                label: name || profession || 'Profissional',
                subtitle: [profession, place].filter(Boolean).join(' · '),
                url: `/perfil/${row.user_id}${row.categories?.slug ? `?categoria=${encodeURIComponent(row.categories.slug)}` : ''}`,
        ...
        const results = [...categoryMap.values(), ...professionalResults]
            .sort((a, b) => a.score - b.score || String(a.label).localeCompare(String(b.label), 'pt-BR'))
            .slice(0, 10)
            .map(({ score: _score, ...item }) => item);

        res.json({ ok: true, results });
    } catch (err) {
        console.warn('Aviso: busca pública incremental indisponível:', err.message || err);
...
app.get("/instalar", catchAsync(async (req, res) => { const { data: banners } = await supabase.from('banners').select('*').eq('is_active', true).order('order', { ascending: true }); res.render("instalar", { banners: normalizeBanners(banners || []), currentPage: 'instalar' }); }));
...
app.get("/categoria/:slug", async (req, res) => {
    try {
        ...
        if (slug === 'outros') {
            const extraCategories = await loadOtherCategories();
            return res.render('outros', { banners, currentPage: 'outros', extraCategories, searchQuery: String(req.query.busca || req.query.q || '').trim() });
        }
